...
add_action('wp_enqueue_scripts', 'brixes_scripts');

function brixes_scripts() {
    global $azexo_azh;
    ...
    if (in_array('administrator', (array) $user->roles)) {
        ...
        $frontend_variables['edit_button'] = esc_html__('Edit', 'brixes');
        ...
        if (function_exists('get_redux_instance')) {
            ...
            $frontend_variables['edit_links']['configure_templates'] = array(
                ...
                'text' => esc_html__('Configure template', 'brixes'),
                'target' => '_blank',
            );
        }
    }
    wp_enqueue_script('brixes-frontend', get_template_directory_uri() . '/js/frontend.js', array('jquery', 'imagesloaded'), AZEXO_FRAMEWORK_VERSION, true);
    wp_localize_script('brixes-frontend', 'azexo', $frontend_variables);
    ...
    if (class_exists('WC_Bookings')) {
        wp_dequeue_style('ui-style');
    }

    if (!empty($options['custom-js'])) {
        wp_add_inline_script('brixes-frontend', $options['custom-js']);
    }
...
add_action('wp_enqueue_scripts', 'brixes_styles');

function brixes_styles() {
    global $azexo_azh;
    ...
    if (class_exists('WPLessPlugin')) {
        ...
        wp_enqueue_style('brixes-skin', $skin_style);
    } else {
    ...
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        ...
        foreach ($options['google_font_families'] as $font_family) {
            if ('off' !== esc_html_x('on', $font_family . ' font: on or off', 'brixes')) {
        ...
        if (!empty($font_families)) {
            ...
            wp_enqueue_style('brixes-fonts', $fonts_url, array(), null);
        }
    }

    wp_enqueue_style('brixes-style', get_stylesheet_uri(), array('brixes-skin'));


    if (!empty($options['custom-css'])) {
        wp_add_inline_style('brixes-style', $options['custom-css']);
    }
    wp_add_inline_style('brixes-style', azexo_dynamic_css());
}

add_action('current_screen', 'brixes_current_screen');

function brixes_current_screen() {
    add_editor_style('editor-styles.css');
}

add_action('init', 'brixes_fix_google_font_families', 13); // after load default skin options

function brixes_fix_google_font_families() {
    $options = get_option(AZEXO_FRAMEWORK);
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        $font_families = array();
        foreach ($options['google_font_families'] as $font_family) {
